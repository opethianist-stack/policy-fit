...
const GENERIC = new Set(['전략', '개선', '지원', '지원사업', '통합', '행사', '지역', '운영', '계획', '사업', '교육', '디지털', 'ai', '인프라', '서비스', '성과', '추진', '개발', '구축', '기반', '확산', '강화', '역량', '연구', '과제', '센터', '플랫폼', '데이터']);
const PLANISH = /(계획|시행계획|기본계획|종합계획|전략|로드맵|방안|정책|추진|공고)/;
// 장·차관 동정성 보도(방문·간담회·축사 등)는 정책 근거가 되기 어렵다 → 감점. 주요정책·사업공고 게시판은 가점
const NEWSY = /(장관|차관|본부장|실장)[^,]{0,20}(방문|참석|간담회|면담|축사|격려|현장|점검|회의|접견|만나|찾아)|기념식|시상식|개소식|출범식|협약식|업무협약|MOU|행사 개최/;
export function msitScore(it, hitCount) {
  return hitCount * 10 + (PLANISH.test(it.title) ? 3 : 0) + (it.files.some((f) => f.readable) ? 1 : 0)
    + (/^policy|^notice$/.test(it.source) ? 4 : 0) - (NEWSY.test(it.title) ? 12 : 0);
}
// loose=true: 흔한 말만 겹친 게시물(느슨한 일치)만 돌려준다. 화면은 접어서 따로 보여 준다
export function matchMsit(items, terms, limit = 8, { loose = false } = {}) {
  const flat = (x) => String(x).toLowerCase().replace(/\s+/g, '');   // 띄어쓰기 무시(검색과 같은 방식)
  const ts = [...new Set(terms.map((t) => String(t).toLowerCase()).filter((t) => flat(t).length >= 2))];
  if (!ts.length) return [];
  // 흔한 말만 겹친 게시물은 개수와 상관없이 느슨한 일치(실측: "지역"+"통합"으로 4극3특 R&D 공고가 본 목록에 옴)
  const isLoose = (x) => x.hit.every((h) => GENERIC.has(flat(h)));
  return items
    .map((it) => {
      const low = flat(it.title);
      const hit = ts.filter((t) => low.includes(flat(t)));
      return { it, hit, score: msitScore(it, hit.length) };
    })
    // 흔한 말 하나만 겹친 게시물은 본 목록에서 뺀다(실측: "전략" 하나로 양자정보과학 과제 선정결과가 뜸)
    .filter((x) => x.hit.length && (loose ? isLoose(x) : !isLoose(x)))
