...
// pdf · hwpx · hwp(5.0) · docx · odt · txt. 스캔 PDF와 배포용(암호화) HWP는 본문을 읽을 수 없다.
...
export function kindOf(name, buf) {
  ...
  if (b[0] === 0x50 && b[1] === 0x4B) return ['docx', 'hwpx', 'odt'].includes(ext) ? ext : 'zip';
...
// odt(오픈문서): content.xml 의 문단(<text:p>, <text:h>) 글자
function fromOdt(files) {
  const x = files['content.xml'];
  if (!x) throw new ExtractError('odt 본문을 찾지 못했습니다.');
  const xml = strFromU8(x);
  const bodyStart = xml.indexOf('<office:body');
  return decodeXml(xml.slice(bodyStart < 0 ? 0 : bodyStart)
    .replace(/<text:tab\/>/g, ' ')
    .replace(/<text:line-break\/>/g, '\n')
    .replace(/<text:s(?: text:c="(\d+)")?\/>/g, (_, n) => ' '.repeat(Number(n || 1)))
    .replace(/<\/text:(?:p|h)>|<\/table:table-cell>/g, '\n')
    .replace(/<[^>]+>/g, ''));
}
...
export async function extractText(buf, name) {
  ...
  else if (kind === 'hwpx' || kind === 'docx' || kind === 'odt' || kind === 'zip') {
    let files;
    try { files = unzipSync(new Uint8Array(buf), { filter: (f) => /^Contents\/section\d+\.xml$|^word\/document\.xml$|^content\.xml$/.test(f.name) }); }
    catch { throw new ExtractError('압축 파일을 열지 못했습니다.'); }
    text = files['word/document.xml'] ? fromDocx(files) : files['content.xml'] ? fromOdt(files) : fromHwpx(files);
  } else if (kind === 'txt') text = buf.toString('utf8');
  else throw new ExtractError('pdf · hwp · hwpx · docx · odt 파일만 읽을 수 있습니다.');
  text = tidy(text);
