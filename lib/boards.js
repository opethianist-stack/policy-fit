...
// NIPA는 전시회·박람회 참가와 입주기업 모집을, KERIS는 채용·점검·결과 발표 같은 공지를 제목으로 뺀다(skip).
// NIA는 입찰을 거의 모두 나라장터(조달청)로 올려 넣지 않는다. 게시판 주소·첨부 호스트는 이 파일에 고정한다(임의 주소를 부르지 않게).
const TTL = 30 * 60 * 1000;
const KEEP_DAYS = 180;   // 게시판 글은 최근 6개월치까지 본다(지난 공고도 참고용으로). 접수 중·예정인 글은 이보다 오래돼도 남긴다
const MAX_PAGES = 10;
const CACHE = new Map();
...
export const BOARDS = {
  kosac: { org: '한국과학창의재단', board: '사업공고', list: (p) => (p === 1 ? KOSAC : `${KOSAC}?page=${p}`), rows: kosacRows,
    idRe: /^PBANC_\d{1,20}$/, view: (id) => `${KOSAC}/${id}`, files: kosacFiles, body: () => '' },
  nipa: { org: '정보통신산업진흥원', board: '사업공고', skip: /전시회|박람회|입주/, list: (p) => (p === 1 ? NIPA : `${NIPA}?curPage=${p}`), rows: nipaRows,
    idRe: /^\d{1,10}$/, view: (id) => `${NIPA}/${id}`, files: nipaFiles, body: nipaBody },
  keris: { org: '한국교육학술정보원', board: '공지사항', skip: KERIS_NOISE, list: (p) => `${KERIS}/selectNttList.do?mi=1051&bbsId=1088&currPage=${p}`, rows: kerisRows,
    idRe: /^\d{1,10}$/, view: kerisView, files: kerisFiles, body: kerisBody },
};

// 한 게시판의 최근 6개월치 글. 쪽의 가장 새 글이 6개월보다 오래되면 멈춘다(맨 위 고정 공지 때문에 가장 새 글로 본다). 30분 캐시.
// 첫 쪽을 못 읽으면 실패, 뒤쪽 실패는 읽은 만큼만
async function boardList(src) {
  const B = BOARDS[src];
  const hit = CACHE.get('list:' + src);
  if (hit && Date.now() - hit.at < TTL) return hit.data;
  const cut = new Date(Date.now() + 9 * 3600e3 - KEEP_DAYS * 864e5).toISOString().slice(0, 10);
  const items = [];
  try {
    for (let p = 1; p <= MAX_PAGES; p++) {
      ...
      for (const x of got) {
        if (items.some((y) => y.id === x.id) || (B.skip && B.skip.test(x.title))) continue;
        if (x.postedAt >= cut || /접수중|접수예정/.test(x.status)) items.push({ src, org: B.org, board: B.board, ...x });
      }
      if (got.every((x) => x.postedAt < cut)) break;
    }
  } catch (e) {
