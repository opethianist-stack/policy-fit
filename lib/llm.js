// Claude API 호출 한 곳. 키는 서버 환경변수에서만 읽고, 응답·오류에 싣지 않는다.
// 답은 자유 글이 아니라 정해진 틀(도구 입력 JSON)로만 받는다 — 칸마다 코드가 검증할 수 있게.
// 키 이름: ANTHROPIC_API_KEY(Vercel) 또는 POLICYFIT_ANTHROPIC_KEY. Claude Code 클라우드 작업 환경은 ANTHROPIC_* 이름을
// 자기 인증용으로 걸러 내거나 자기 값으로 채우므로(ANTHROPIC_BASE_URL) 그 환경에서는 POLICYFIT_* 이름을 쓴다.
// 주소를 바꾸는 건 시험용 가짜 서버뿐이라 POLICYFIT_ANTHROPIC_BASE_URL만 읽는다.
const BASE = process.env.POLICYFIT_ANTHROPIC_BASE_URL || 'https://api.anthropic.com';
export const MODEL = process.env.ANTHROPIC_MODEL || process.env.POLICYFIT_ANTHROPIC_MODEL || 'claude-haiku-4-5-20251001';
const apiKey = () => process.env.ANTHROPIC_API_KEY || process.env.POLICYFIT_ANTHROPIC_KEY || '';

export const llmReady = () => !!apiKey();
