...
export function buildDraft({ notice, lineage, cards, headlines = {}, conclusion = '', copy = null }) {
  ...
  sections.push({
    ...
    blocks: used.map((r, i) => ({
      ...
      bullets: ok.filter((c) => c.role === r.key).map((c) => ({
        cardId: c.id, quoted: !c.text, text: c.text || tidy(c.quote), cite: cite(c),
      })),
    })),
  });
  ...
  return {
    ...
    factWarnings,
    blocked,
    deck: recommendDeck(ok, numeric, used, chain, head, concl, notice, copy),
  };
}
...
function recommendDeck(ok, numeric, used, chain, head, concl, notice, copy) {
  const T = (copy && copy.title) || {}, SUM = (copy && copy.summary) || {};
  const cut = (s, n) => { s = clean(s); return s.length > n ? s.slice(0, n - 1) + '…' : s; };
  const line = (c, n) => (c.text ? { text: cut(c.text, n), quoted: false, terms: [] } : SUM[c.id] ? { text: cut(SUM[c.id], n), quoted: false, terms: [], ai: true } : { text: cut(tidy(c.quote), n), quoted: true, terms: c.matched || [] });
  ...
  if (stageCards.length >= 2) {
    options.push({
      ...
      slide: {
        ...
        stages: stageCards.map((c) => ({ year: c.year, title: stageTitle(c), top: SUM[c.id] ? { text: cut(SUM[c.id], 46), quoted: false, terms: [], ai: true } : quote(c, 46), bottom: c.text ? { text: cut(c.text, 46) } : null, source: src(c) })),
