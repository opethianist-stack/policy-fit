...
function recommendDeck(ok, numeric, used, chain, head, concl, notice, copy, lineageMinistry) {
  ...
  // 로드맵 왼쪽 세로칸용 짧은 이름. 5자 이하 그대로, 교육청은 "교육청", 5자 이하 약칭(과기정통부·KERIS)이 있으면 그것, 없으면 null
  const shortOrg = (name) => {
    name = fullOrg(name || '');
    if (!name) return null;
    if (name.length <= 5) return name;
    if (/교육(지원)?청$/.test(name)) return '교육청';
    const alias = Object.keys(rules.aliases).find((k) => k.length <= 5 && (rules.aliases[k] || [])[0] === name && !/교육청$/.test(k));
    return alias || null;
  };
