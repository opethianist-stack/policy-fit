...
// 장표 구도 추천(규칙). 제안서 발표자료에서 쓰는 사업 이해도 장표 구도 중 근거 구성에 맞는 것을 골라,
// 실제 문구(헤드라인·발췌·수렴점·출처)를 채운 16:9 목업 데이터로 돌려준다. 그리기는 화면이 한다.
//   converge  수렴형      역할별 3~4단 + 하단 수렴점 바
//   quadrant  사분면형    헤드라인 + 2×2 칸
//   flow      배경-해결형 배경(정책 환경/현장 여건) → 해결 배너 → 발주처 계보 흐름
//   numeric   수치 강조형 핵심 수치 2~3개 + 근거 표
function recommendDeck(ok, numeric, used, chain, head, concl, notice) {
  const cut = (s, n) => { s = clean(s); return s.length > n ? s.slice(0, n - 1) + '…' : s; };
  const line = (c, n = 64) => ({ text: cut(c.text || c.quote, n), quoted: !c.text });
  const ofRole = (k) => ok.filter((c) => c.role === k);
  const block = (r, n = 2, len = 64) => ({ label: r.side.replace(/ 측$/, ''), headline: head(r.key), bullets: ofRole(r.key).slice(0, n).map((c) => line(c, len)) });
  const docs = [];
  ...
  // 수렴점이 따로 칸을 갖는 구도는 제목에 사업명을, 수치 강조형은 수렴점을 제목으로 쓴다(같은 문장이 두 번 나오지 않게)
  const base = { kicker: '사업 이해도', title: name, foot };

  const options = [];
  if (used.length >= 2) {
    options.push({
      key: 'converge', name: '수렴형', when: '서로 다른 측면의 근거가 하나의 추진 필요성으로 모일 때',
      slide: { ...base, layout: 'converge', columns: used.map((r) => block(r, 2, used.length > 3 ? 48 : 60)), bottom: { label: '수렴점', text: concl } },
    });
  }
  const cells = used.map((r) => block(r, 2, 56));
  if (concl && cells.length < 4) cells.push({ label: '사업 추진의 필요성', headline: concl, bullets: [] });
  if (cells.length >= 4) {
    options.push({
      key: 'quadrant', name: '사분면형', when: '근거 측면이 넷이거나, 셋에 수렴점까지 갖춰졌을 때',
      slide: { ...base, layout: 'quadrant', cells: cells.slice(0, 4) },
    });
  }
  const group = (keys) => ok.filter((c) => keys.includes(c.role));
  const background = [
    { label: '정책 환경', keys: ['policy', 'agency'] },
    { label: '현장 여건', keys: ['tech', 'demand'] },
  ].map((g) => ({ label: g.label, bullets: group(g.keys).slice(0, 2).map((c) => line(c, 70)) })).filter((g) => g.bullets.length);
  options.push({
    key: 'flow', name: '배경-해결형', when: '정책 배경에서 사업의 해결 방향으로 이어지는 흐름을 보여줄 때',
    slide: { ...base, layout: 'flow', background, solution: concl, steps: chain.map((c) => ({ name: c.name, role: c.role })) },
  });
  if (numeric.length >= 2) {
    const G = new RegExp(NUMERIC.source, 'g');
    const stats = numeric.slice(0, 3).map((c) => {
      ...
      return { value: m[0].replace(/\s/g, ''), label: cut(ctx, 46), source: `「${c.title}」 p.${c.page}` };
    });
    const rest = ok.filter((c) => !numeric.slice(0, 3).includes(c)).slice(0, 3);
    options.push({
      key: 'numeric', name: '수치 강조형', when: '수치가 들어간 근거가 2건 이상일 때',
      slide: { ...base, layout: 'numeric', title: concl || name, stats, rows: rest.map((c) => ({ label: (ROLES.find((r) => r.key === c.role) || {}).label || '', ...line(c, 70), source: `「${c.title}」 p.${c.page}` })) },
    });
  }

  const rec = numeric.length >= 3 ? 'numeric' : used.length >= 3 ? 'converge' : 'flow';
  const reason = rec === 'numeric'
    ? `수치가 들어간 근거가 ${numeric.length}건이라 숫자를 앞세우는 구도를 추천합니다.`
    : rec === 'converge'
      ? `근거가 ${used.map((r) => r.label).join(' · ')}에 걸쳐 있어 한 결론으로 모으는 구도를 추천합니다.`
      : '근거 측면이 둘 이하라 배경에서 해결 방향으로 이어지는 구도를 추천합니다.';
