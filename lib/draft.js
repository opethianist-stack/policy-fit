...
function recommendDeck(ok, numeric, used, chain, head, concl, notice, copy) {
  ...
  const line = (c, n) => (c.text ? { text: cut(c.text, n), quoted: false, terms: [] } : SUM[c.id] ? { text: cut(SUM[c.id], n), quoted: false, terms: [], ai: true } : { text: cut(c.quote, n), quoted: true, terms: c.matched || [] });
