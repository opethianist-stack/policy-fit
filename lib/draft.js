...
// 원문 쪽 발췌 창: 발췌가 걸친 줄을 가운데 두고 앞뒤 몇 줄을 원문 그대로 돌려준다(칸 구분 EN SPACE는 공백으로, 긴 줄은 자름).
// 발췌 줄은 hit 로 표시해 화면이 강조 테두리를 그린다. 원문 쪽을 못 찾으면 null
function pageWindow(c, before, after, width, max) {
  const p = getPage(c.docId, c.page);
  if (!p) return null;
  const sq = (x) => x.replace(/\s+/g, '');
  const lines = String(p.text).split('\n').map((l) => clean(l.replace(/\u2002/g, ' '))).filter(Boolean);
  const offs = []; let acc = '';
  for (const l of lines) { offs.push(acc.length); acc += sq(l); }
  const at = acc.indexOf(sq(c.quote || ''));
  if (at < 0) return null;
  const endAt = at + sq(c.quote).length;
  let start = -1, end = -1;
  lines.forEach((l, i) => { const s0 = offs[i], e0 = s0 + sq(l).length; if (e0 > at && s0 < endAt) { if (start < 0) start = i; end = i; } });
  end = Math.min(end, start + max - 2);
  const from = Math.max(0, start - before), to = Math.min(lines.length, end + after + 1, from + max);
  const cutw = (t) => (t.length > width ? t.slice(0, width - 1) + '…' : t);
  return { page: c.page, lines: lines.slice(from, to).map((t, k) => ({ text: cutw(t), hit: from + k >= start && from + k <= end })) };
}
...
//   derive   근거 도출형     왼쪽 주 근거 원문 쪽 → 오른쪽 4칸(정책 연계·측면별 요지·수치 근거·사업 방향)
//   numeric  수치 강조형     핵심 수치 + 근거 표
//   roadmapDocs · survey · link · plan · pillars · spotlight · approach  — 아래 각 블록 주석 참고(부서 Template 폴더 장표)
...
function recommendDeck(ok, numeric, used, chain, head, concl, notice, copy, lineageMinistry) {
  ...
  // roadmapDocs: 찾아가는 학교 컨설팅 "지금 세계는"(상징 + 짧은 설명을 가로로) · 교실혁명 "디지털 기반 교육 전환 로드맵"(연도 점)
  // · 교실혁명 "공교육 혁신의 골든 타임"(보도자료 → 체크 시사점)을 합쳤다. 연도 흐름 선 위에 정책 문서 표지를 늘어놓고,
  // 문서마다 요지 한 줄, 아래는 측면별 시사점 체크와 사업 추진의 필요성. 로드맵형과 달리 단계 카드·제안사 행이 없다
  const dseen = new Set();
  const lineDocs = ok.filter((c) => c.role !== 'demand')
    .sort((a, b) => ROLE_ORDER[a.role] - ROLE_ORDER[b.role])
    .filter((c) => (dseen.has(c.docId) ? false : dseen.add(c.docId))).slice(0, 4)
    .sort((a, b) => String(a.year).localeCompare(String(b.year)));
  if (lineDocs.length >= 2) {
    const lineMin = !!ministry && lineDocs.some((c) => c.org === ministry || fullOrg(c.org) === ministry);
    options.push({
      key: 'roadmapDocs', name: '로드맵·근거문서형', when: '근거로 삼은 정책 문서를 연도순으로 늘어놓아 흐름과 출처를 함께 보여줄 때',
      slide: {
        ...base, layout: 'docsline',
        band: lineMin ? `${ministry} 정책 흐름` : '정책 흐름',
        docs: lineDocs.map((c) => ({ year: c.year, title: cut(c.title, 24), org: fullOrg(c.org), role: roleOf(c.role).label, point: line(c, 58), page: c.page })),
        checks: used.slice(0, 3).map((r) => ({ label: r.label, headline: head(r.key) })),
        bottom: concl,
      },
    });
  }
  ...
  {   // 설문 막대형은 틀 위주라 근거 구성과 관계없이 늘 낸다(현장 수요·수치 카드가 있으면 참고 수치가 채워진다)
  ...
  // 문서가 하나뿐이어도 카드가 2장 이상이면 카드 단위로 칸을 나눈다(한 문서의 서로 다른 쪽)
  if (linkCards.length < 2) linkCards.splice(0, linkCards.length, ...ok.slice(0, 3));
  ...
  // 수치 근거: 수치가 든 카드마다 대표 숫자 하나(같은 숫자 중복 제거). 수치 강조형·근거 도출형이 함께 쓴다
  const G = new RegExp(NUMERIC.source, 'g');
  const statSeen = new Set();
  const statAll = numeric.map((c) => {
  ...
  }).filter((x) => (statSeen.has(x.value) ? false : statSeen.add(x.value)));
  const plainStat = (x) => ({ value: x.value, label: x.label, source: x.source });

  // approach: 기업 연계 정보교원 "사업 배경 및 목적"을 가공한 인천 "제안 목적 및 범위 ①" — 왼쪽 BACKGROUND(근거 문서 2건: 측면 알약 +
  // 변화 한 줄 + 원문 쪽 발췌 창 + 문서명·요지), 오른쪽 OUR APPROACH(제안사 운영 모델이라 빈칸, 슬로건 설명만 수렴점)
  const aseen = new Set();
  const bgCards = [...ok].sort((a, b) => (a.role === 'demand') - (b.role === 'demand') || ROLE_ORDER[a.role] - ROLE_ORDER[b.role])
    .filter((c) => (aseen.has(c.docId) ? false : aseen.add(c.docId))).slice(0, 2);
  if (bgCards.length) {
    const usedHeadA = new Set();
    options.push({
      key: 'approach', name: '정책 접근형', when: '정책 근거(배경)와 제안사의 접근 방식(운영 모델)을 좌우로 맞세울 때',
      slide: {
        ...base, layout: 'approach',
        bg: bgCards.map((c) => {
          const h = head(c.role);
          const shift = h && !usedHeadA.has(c.role) ? (usedHeadA.add(c.role), cut(h, 28)) : '';
          return { pill: T[c.id] ? cut(T[c.id], 10) : roleOf(c.role).label, shift, win: pageWindow(c, 1, 3, 44, 7), cite: `「${cut(c.title, 26)}」(${fullOrg(c.org)}, ${c.year})`, sub: line(c, 60) };
        }),
        sub: concl,
        keys: 3, units: 5,
      },
  ...
  // derive: 인천 "제안 목적 및 범위 ②"(기업 연계 정보교원 장표 가공) — 왼쪽 어두운 판에 주 근거 문서 원문 쪽(발췌 줄 강조 테두리),
  // 오른쪽 점선 안 4칸: 정책 연계(상위 → 발주 → 본 사업) · 측면별 요지(알약) · 수치 근거(없으면 현장 수요 요지) · 사업 방향(헤드라인 → 수렴점)
  const mainCard = ofRole('agency')[0] || ok.find((c) => c.role !== 'demand') || ok[0];
  if (mainCard) {
    const flowCards = [];
    for (const k of ['policy', 'tech', 'agency']) { const c = ofRole(k).find((x) => !flowCards.some((y) => y.docId === x.docId)); if (c && flowCards.length < 2) flowCards.push(c); }
    const stepName = (c) => shortOrg(c.org) || cut(fullOrg(c.org), 10);   // 가로 글이라 영문 약칭 대신 정식 이름(10자)
    const flowPath = [...flowCards.map(stepName), '본 사업'].filter((x, i, a) => a.indexOf(x) === i).join(' → ');
    const mainPages = [...new Set(ok.filter((c) => c.docId === mainCard.docId).map((c) => c.page))].sort((a, b) => a - b).slice(0, 4);
    const dStats = statAll.slice(0, 3).map(plainStat);
    const demandLines = [...demand, ...ofRole('tech')].slice(0, 3).map((c) => ({ ...line(c, 44), source: src(c) }));
    const dirs = used.map((r) => head(r.key)).filter(Boolean).slice(0, 2).map((t) => ({ text: cut(t, 34) }));
    options.push({
      key: 'derive', name: '근거 도출형', when: '발주기관·상위 정책 원문 한 쪽을 앞세우고 연계·요지·수치·방향을 칸으로 나눠 도출할 때',
      slide: {
        ...base, layout: 'derive',
        doc: { title: `「${cut(mainCard.title, 30)}」`, meta: `${fullOrg(mainCard.org)} · ${mainCard.year}`, win: pageWindow(mainCard, 3, 9, 52, 16), caption: `「${cut(mainCard.title, 30)}」 p.${mainCard.page} 원문` },
        flow: { path: flowPath, steps: flowCards.map((c) => ({ title: `${stepName(c)} 「${cut(c.title, 16)}」`, sub: line(c, 30) })), last: { title: '본 사업 진행', sub: cut((notice && notice.name) || '', 30) } },
        aims: used.slice(0, 3).map((r) => ({ label: r.label, text: head(r.key) })),
        stats: dStats.length ? { tag: '수치 근거', items: dStats } : { tag: '현장 수요', lines: demandLines },
        dirs, bottom: concl,
        srcNote: `※ 「${mainCard.title}」(${fullOrg(mainCard.org)}, ${mainCard.year}) p.${mainPages.join(' · p.')}`,
      },
    });
  }
  if (numeric.length >= 2) {
    const stats = statAll.slice(0, 3);
    ...
    if (stats.length >= 2) options.push({
      ...
      slide: { ...base, layout: 'numeric', title: '핵심 지표', stats: stats.map(plainStat), rows: rest.map((c) => ({ label: roleOf(c.role).label, ...line(c, 70), source: src(c) })) },
    });
  }
  ...
  const reason = {
    ...
    derive: '근거가 적어 주 근거 문서 한 쪽을 앞세우고 연계·요지·방향을 도출하는 구도를 추천합니다.',
  }[rec];
  options.sort((a, b) => (a.key === rec ? -1 : b.key === rec ? 1 : 0));
  const aiUsed = !!copy && ok.some((c) => T[c.id] || SUM[c.id]);
  // 조건이 안 맞아 빠진 구도와 그 조건. 화면이 탭 아래에 알려 준다
  const NEED = [
    ['roadmap', '정책 로드맵형', '현장 수요가 아닌 서로 다른 문서 2건 이상'],
    ['roadmapDocs', '로드맵·근거문서형', '현장 수요가 아닌 서로 다른 문서 2건 이상'],
    ['plan', '발주기관 계획형', '발주기관 계획 역할 카드 2건 이상'],
    ['converge', '수렴형', '역할 2개 이상'],
    ['pillars', '측면 원형형', '역할 2개 이상'],
    ['voices', '현장 근거형', '카드 3건 이상 + 역할 2개 이상'],
    ['link', '정책-연결형', '카드 2건 이상'],
    ['numeric', '수치 강조형', '서로 다른 수치가 든 카드 2건 이상'],
    ['spotlight', '정책 인용형', '현장 수요가 아닌 카드 1건 이상'],
  ];
  const hidden = NEED.filter(([k]) => !options.some((o) => o.key === k)).map(([key, name, need]) => ({ key, name, need }));
  return { recommended: rec, reason, options, hidden, aiUsed };
}
