...
export async function draftText(body) {
  ...
  const user = [
    ...
    `- sentences: 카드마다 개조식 불릿을 카드 제목 줄의 "불릿 N개"만큼 쓴다. 불릿 하나는 한 문장, ${BULLET_MAX - 10}자 이내. 발췌에 든 사실을 빠짐없이 나눠 담고, 불릿끼리 같은 내용을 되풀이하지 않는다. 그 카드 발췌에 있는 내용만. 불릿마다 sentences 항목 하나(같은 id를 여러 번 쓴다). id는 카드 앞 대괄호 안 번호(C1, C2 …) 그대로.`,
    '- headlines: 역할마다 헤드라인 1개(30자 이내). 그 역할 카드들의 공통 주장.',
    '- conclusion: 모든 근거가 모이는 "사업 추진의 필요성" 1~2문장(100자 이내).',
    '',
    byRole.map((g) => `## ${roleLabel(g.key)}\n` + g.cards.map((c) => cardBlock(c, false).replace(/^(\[C\d+\][^\n]*)/, `$1 · 불릿 ${bulletsFor(c.quote)}개`)).join('\n\n')).join('\n\n'),
  ].join('\n');
  ...
  for (const s of input.sentences || []) {
    ...
    const list = got.get(c.id) || [];
    const t = trim(s.text);
    if (list.length >= bulletsFor(c.quote) || list.includes(t)) continue;
    const why = checkText(t, c.pageText, { max: BULLET_MAX, allowed });
    if (why) dropped.push({ where: `문장 ${c.id}`, reason: why }); else { list.push(t); got.set(c.id, list); }
  }
  // 화면·문서에는 줄바꿈으로 이어 한 칸에 넣는다(줄 하나 = 불릿 하나)
  const sentences = [...got].map(([id, list]) => ({ id, text: list.join('\n'), bullets: list.length }));
  const headlines = [];
