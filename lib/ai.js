...
// ---------- 2-2. 정책 근거 관련도 정렬 ----------
// 키워드 검색 후보(최대 30건)를 사업과의 관련도로 다시 줄 세운다. LLM은 후보에 점수·이유만 붙인다 —
...
export async function rankEvidence({ notice, lineage, rfpText = '', candidates = [] }) {
  ...
  const user = [
    ...
    'why는 판단 이유 25자 이내(예: "직업계고 AI 교육 지원 계획 직접 언급"). 모든 후보에 빠짐없이 답한다. id는 대괄호 안 번호 그대로.',
    '',
    list.map((x) => `[${x.alias}] ${fullOrg(x.c.org)} 「${x.c.title}」(${x.c.year}) p.${x.c.page}\n발췌: ${trim(x.c.quote).slice(0, 300)}`).join('\n\n'),
  ].filter((v) => v !== '').join('\n');
