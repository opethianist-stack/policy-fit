...
import { verifyQuote, getPage, pageFreq, squash } from './search';
...
const cardBlock = (c, withRole = true) =>
  `[${c.id}] ${c.orgFull} 「${c.title}」(${c.year}) p.${c.page}${withRole && c.role ? ` · 역할: ${roleLabel(c.role)}` : ''}\n발췌: ${c.quote}${c.text ? `\n담당자 문장: ${c.text}` : ''}`;
...
export async function classifyRoles({ cards, lineage, notice }) {
  ...
  const tool = {
    ...
    input_schema: {
      type: 'object',
      properties: { roles: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, role: { type: 'string', enum: ROLE_KEYS }, why: { type: 'string' } }, required: ['id', 'role'] } } },
      required: ['roles'],
    },
  };
  const user = [
    ...
    'agency = 발주기관 계획(발주기관이나 그 상위 기관의 자체 사업·계획)',
    'why는 15자 이내.',
    '',
  ...
  for (const r of input.roles || []) {
    if (!r || !ids.has(String(r.id))) { dropped.push({ id: r && r.id, reason: '없는 카드' }); continue; }
    if (!ROLE_KEYS.includes(r.role)) { dropped.push({ id: r.id, reason: '없는 역할' }); continue; }
    out.push({ id: String(r.id), role: r.role, why: trim(r.why).slice(0, 30) });
  }
...
export async function draftText(body) {
  ...
  const tool = {
    ...
    input_schema: {
      type: 'object',
      properties: {
        sentences: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, text: { type: 'string' } }, required: ['id', 'text'] } },
  ...
  const user = [
    ...
    '- sentences: 카드마다 개조식 문서 문장 1개(80자 이내). 그 카드 발췌에 있는 내용만.',
  ...
  for (const s of input.sentences || []) {
    const c = s && byId.get(String(s.id));
...
export async function deckCopy(body) {
  ...
  const tool = {
    ...
    input_schema: {
      type: 'object',
      properties: {
        headline: { type: 'object', properties: { line1: { type: 'string' }, line2: { type: 'string' } }, required: ['line1', 'line2'] },
        cells: { type: 'array', items: { type: 'object', properties: { id: { type: 'string' }, title: { type: 'string' }, summary: { type: 'string' } }, required: ['id', 'title', 'summary'] } },
      },
  ...
  const user = [
    ...
    `- headline: 장표 가운데 두 줄 헤드라인. line1은 "~을 통한"처럼 수단·배경(${DECK_LIMITS.line}자 이내), line2는 핵심 메시지(${DECK_LIMITS.line}자 이내). 예) line1 "디지털 전환 기반 교원 역량 강화를 통한" / line2 "교수학습 혁신 및 맞춤형 교육 실현"`,
    `- cells: 카드마다 title(정책 단계 이름, ${DECK_LIMITS.title - 2}자 안팎, 예 "디지털 교육 체제 전환")과 summary(칸 요약, ${DECK_LIMITS.summary - 4}자 안팎, 예 "교육부 디지털 기반 교육 혁신 방안 발표").`,
    '',
    byRole.map((g) => `## ${roleLabel(g.key)}\n` + g.cards.map((c) => cardBlock(c, false)).join('\n\n')).join('\n\n'),
  ].join('\n');
  ...
  if (input.headline) {
    ...
    const w1 = checkText(input.headline.line1, src, { max: DECK_LIMITS.line, allowed });
    const w2 = checkText(input.headline.line2, src, { max: DECK_LIMITS.line, allowed });
    if (w1 || w2) dropped.push({ where: '헤드라인', reason: w1 || w2 });
    else headline = [trim(input.headline.line1), trim(input.headline.line2)];
  }
  const cells = [];
  for (const x of input.cells || []) {
    const c = x && byId.get(String(x.id));
