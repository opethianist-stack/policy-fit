...
최종 갱신: 2026-09-21 (STEP 3-4 보정: 제안요청서 검색어·발주기관 추정, 직전 커밋 `db28cab`)
...
- **검색어 뽑기**(`lib/rfp.js`): `rfpCounts`(제안요청서 안 가중 빈도) → `termsFromRfp`(색인 대조·점수)
  ...
  - **점수**: 제안요청서 빈도 × 색인 희소성 × 색인 지지도(색인 4쪽 미만에 나오는 말은 비례 감점 — 한두 번 우연히 겹친 말이 위로 오지 않게). 색인 쪽 빈도 25% 이상인 흔한 말과 색인에 없는 말은 뺀다. 공고명 검색어에 포함된 말("AI동행" → "동행")도 뺀다. 상위 8개
