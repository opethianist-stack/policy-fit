...
최종 갱신: 2026-09-21 (STEP 3-1·3-2 반영, 직전 커밋 `9d00def`)
...
1. **제안요청서 입력 (STEP 3-4)** — 홈에 업로드 입구(나라장터 밖 사업용) + 공고 첨부파일 자동 수집 가능 여부 확인. 업로드 파일은 저장하지 않고 텍스트만 뽑아 세션 동안 쓴다. 지금 검색 재료가 공고명뿐이라 빈약한 공고명에서 결과가 빈약하다. 검색 품질 개선 효과가 가장 큰 항목
