...
| `/prototype` | `app/prototype/page.js` → `public/prototype.html` (iframe) | 사용자 화면 5종(홈·정책 근거 검색·근거 카드 챗봇·문서 산출·구도 추천). 바닐라 HTML/JS. **홈·정책 근거 검색·근거 카드 검토는 실데이터**(아래 API 3종)로 동작한다. 문서 산출·구도 추천은 아직 목업 |
...
**8단계 중 ①공고 조회 ②계보 매핑 ③근거 검색 ④카드 생성(발췌) ⑤검토까지 실데이터로 돈다. ⑥초안 ⑦문서 산출 ⑧구도 추천은 목업이다.**
...
1. **2단계: 문서 산출** — 채택 카드 + 연결 논리로 "사업 이해도·추진 배경" docx 생성. 발췌문마다 출처(문서명·연도·쪽) 표기. 구도 추천은 채택 카드 수·유형에 따른 규칙
...
7. **LLM 적용 지점** — 검색어 추출, 연결 논리 초안. 발췌·검증은 규칙 기반 유지
8. **`/flow` 탭** — 업무 플로우 다이어그램 페이지. 배포 용량 제약 때문에 뺐던 것이라 지금은 넣어도 된다. 넣기 전까지는 README 구성표의 `/flow` 행이 실제와 다르다
