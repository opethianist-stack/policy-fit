...
최종 갱신: 2026-09-21 (과기정통부 API 4종 연결 · 구글드라이브 색인 자동 갱신 · 검색어 10개 제한, 직전 커밋 `4cec7c7`)
...
3. **드라이브 동기화 첫 실행 확인** — Actions 수동 실행 → 색인 커밋·원문 링크 확인. 이후 과기부 첨부 자동 수집(주요정책 01·03·10 + 제목 필터 → 첨부 받기 → odt면 PDF 변환, hwpx면 직접 파싱 → 드라이브 `_auto/msit/`에 보관 → 색인)을 같은 워크플로에 붙인다(`DATA_GO_KR_KEY` Secret 등록됨)
