...
| `/prototype` | `app/prototype/page.js` → `public/prototype.html` (iframe) | 사용자 화면 5종(홈·정책 근거 검색·근거 카드 챗봇·문서 산출·구도 추천). 바닐라 HTML/JS. **다섯 화면 모두 실데이터**(아래 API)로 동작한다 |
...
| `/api/evidence?title=&orgs=&terms=&scope=` | `app/api/evidence/route.js` → `lib/search.js` | 색인에서 쪽 단위 검색 후 발췌문 반환. 외부 호출·LLM 없음 |
| `/api/draft` (POST) | `app/api/draft/route.js` → `lib/draft.js` | 채택 카드 + 연결 논리 → 문서 모델(JSON)과 구도 추천. 화면 미리보기가 이걸 그린다 |
...
발췌문은 색인 본문에서 잘라낸 원문이고, 반환 직전에 공백을 무시한 원문 포함 여부를 다시 대조해 불일치면 버린다. "연결 논리"는 검토 화면에서 담당자가 직접 쓴다. LLM은 아직 붙이지 않았다.

**⑧ 문서 산출과 구도 추천 (`lib/draft.js`)**

화면 미리보기와 docx가 같은 문서 모델 하나에서 나온다. 문서에 들어가는 문장은 세 종류뿐이다: ① 색인 원문과 대조를 통과한 발췌문 ② 담당자가 쓴 연결 논리 ③ API 값으로 채운 정형 문장(사업 개요 표, 계보). 산출 직전에 발췌문을 색인과 다시 대조하고, 실패한 카드는 문서에 넣지 않고 `blocked`로 돌려준다. 이것이 "출처 미확보 문장 차단"의 구현이다.

문서 구성은 `1. 사업 개요 → 2. 발주처와 정책 계보 → 3. 정책 근거(범부처 · 주관부처 · 발주기관 · 관련 기관 순) → 4. 근거 요약`. 산출물에는 도구 이름이나 생성 경위 같은 메타 서술을 넣지 않는다.

구도 추천은 규칙이다. 수치가 들어간 근거가 2건 이상이면 "수치 강조형", 아니면 "계보 흐름형"을 추천하고, 각 안의 칸에 어떤 근거를 놓을지까지 채운다. AI미래교육본부 PPT 템플릿 실물은 아직 반영하지 못했다(칸 이름이 템플릿의 실제 레이아웃과 맞는지 확인 필요).
...
1. **실사용 점검** — 실제 공고 몇 건으로 끝까지 돌려보고 검색 품질·발췌문 끊김·문서 구성을 고친다
2. **원문 링크** — 정책문서를 구글드라이브에 올리고 `data/corpus-links.json`을 채운다. 검토 단계의 원문 대조가 이걸로 완성된다
3. **교육부 보도자료 아카이빙** — 링크 취합 → 구글 스프레드시트 목록화 → 첨부파일 구글드라이브 저장
4. **지식베이스 스토리지 구성** — 구글드라이브를 코퍼스 저장소로 확정하고 적재 규칙 정하기
5. **스캔 PDF OCR** — `14-1`, `14-2` 두 건. HWP·HWPX 본문 추출은 해결됨
6. **제안요청서 입력** — 나라장터에 없는 사업용 업로드 입구, 그리고 공고 첨부파일(제안요청서) 본문에서 검색어를 뽑는 단계. 지금은 공고명만 쓴다
7. **hwpx 산출**, **PPT 템플릿 실물 반영**(구도 추천의 칸 구성)
8. **LLM 적용 지점** — 검색어 추출, 연결 논리 초안. 발췌·검증은 규칙 기반 유지
