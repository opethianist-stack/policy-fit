...
최종 갱신: 2026-09-21 (장표 구도 추천을 16:9 목업으로 개편, 직전 커밋 `efafeff`)
...
8. **장표 구도 추천** — 근거 구성에 맞는 사업 이해도 장표 구도를 실제 문구가 채워진 16:9 HTML 목업으로 보여준다 (PPT 파일 제작은 범위 밖)
...
**구도 추천(2026-09-21 개편)** — 예전 안1·안2 칸 목록은 장표와 거리가 멀어 **16:9 장표 목업(HTML)**으로 바꿨다. PPT 파일을 만들지 않고 화면에서 그린다(토큰·비용 없음). "AI미래교육본부 PPT 템플릿 기반" 표시는 삭제했다(실물 반영 전이라 사실과 달랐음).

- 서버(`recommendDeck`)는 구도별 장표 데이터(`slide`)를 돌려주고, 화면(`prototype.html`의 `slideHtml`)이 그린다. 장표 안 글자는 문서와 같은 네 종류뿐(헤드라인·담당자 문장·대조 통과 발췌·출처)
- 구도 4종 — 실제 제안 발표자료에서 가져온 패턴

  | key | 이름 | 근거 패턴 | 나오는 조건 |
  |---|---|---|---|
  | `converge` | 수렴형 | KICE 제안서 수렴 표(기술 공급·정책·현장 수요 → 수렴점) | 역할 2개 이상 |
  | `quadrant` | 사분면형 | 서울 AI디지털배움터 발표 "사업 이해도 ①" 2×2 | 역할 4개, 또는 역할 3개 + 수렴점 |
  | `flow` | 배경-해결형 | KMA 정보교원 발표 "사업 배경 및 목적"(BACKGROUND → SOLUTION → 흐름) | 항상 |
  | `numeric` | 수치 강조형 | 핵심 수치 카드 + 근거 표 | 수치 든 근거 2건 이상 |

- 추천 규칙: 수치 든 근거 3건 이상 → 수치 강조형, 역할 3개 이상 → 수렴형, 나머지 → 배경-해결형. 추천안이 맨 위(보라 테두리 + "추천")
- 장표 제목은 사업명. 수치 강조형만 수렴점을 제목으로 쓴다(수렴점 칸이 따로 있는 구도에서 같은 문장이 두 번 나오지 않게). 배경-해결형의 "정책 환경" = 정책 기조 + 발주기관 계획, "현장 여건" = 기술·환경 + 현장 수요. 하단 흐름은 발주처 계보
- 하단 각주 `* 출처: 「문서명」(기관 정식명, 연도) | …` 3건 + 외 N건 (KMA 발표자료 각주 형식)
- 비어 있는 칸은 "헤드라인 입력 전", "사업 추진의 필요성 입력 전"을 흐리게 표시한다
- 크기는 장표 너비 기준 단위(`cqw`)라 카드 폭에 따라 같이 줄어든다. 폭 560px 미만(휴대폰)에서는 가로 스크롤
- 수치 카드의 숫자는 발췌문에서 `NUMERIC` 규칙으로 뽑은 첫 수치다. 담당자 문장이 있으면 그 문장이 설명줄이 된다
